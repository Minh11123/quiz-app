...
public class QuestionController {
    ...
    public ResponseEntity<?> addQuestion(@RequestBody Question question){
        System.out.println("day la link nhan dc +" + question.getImage());
//        question.setImage("okok link + " + question.getImage());
        if(getDriveFileId(question.getImage()) != null){
            question.setImage("https://drive.google.com/uc?export=download&id=" + getDriveFileId(question.getImage()));
        }
        return  new ResponseEntity<>(questionService.save(question), HttpStatus.OK);
    }


    public String getDriveFileId(String driveLink) {
...
//        Collections.shuffle(questions);
    ...
    @GetMapping("/quiz/listen/{quizId}")
    public ResponseEntity<?> getQuestionsOfQuizForTestListening(@PathVariable Long quizId){
//        Quiz quiz = new Quiz();
//        quiz.setId(quizId);
//        return new ResponseEntity<>(questionService.getQuestionsByQuiz(quiz),HttpStatus.OK);
        // lấy luôn questions từ quiz và kiểm tra độ dài maxquiz rồi trả về sao cho đúng ngần ý câu
